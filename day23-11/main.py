...
for index,element in enumerate(empty_rows):
    ...
    empty_rows[index+1:] = [x+1 for x in empty_rows[index+1:]]
    for key in galaxies_dict:
        if galaxies_dict[key][0] > element:
            galaxies_dict[key] = (galaxies_dict[key][0]+1, galaxies_dict[key][1])

for index,element in enumerate(empty_columns):
    empty_columns[index + 1:] = [x + 1 for x in empty_columns[index + 1:]]
    for key in galaxies_dict:
        if galaxies_dict[key][1] > element:
            galaxies_dict[key] = (galaxies_dict[key][0], galaxies_dict[key][1]+1)
